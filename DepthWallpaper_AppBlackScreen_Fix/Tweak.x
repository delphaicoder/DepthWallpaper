...
static void DW_LoadMetadata(CGFloat *outYOffsetRatio, CGFloat *outScale, BOOL *outEnabled) {
    NSDictionary *meta = [NSDictionary dictionaryWithContentsOfFile:DWMetadataPath];
    *outYOffsetRatio = meta[DWMetaKeyYOffsetRatio] ? [meta[DWMetaKeyYOffsetRatio] doubleValue] : 0.30;
    *outScale        = meta[DWMetaKeyScale]        ? [meta[DWMetaKeyScale] doubleValue]        : 1.0;
...
- (void)reloadImage {
    CGFloat yRatio, scale; BOOL enabled;
    DW_LoadMetadata(&yRatio, &scale, &enabled);
    ...
    CGFloat imgAspect = img.size.width / MAX(img.size.height, 1);
    CGFloat w = screenBounds.size.width * scale;
    CGFloat h = w / imgAspect;
    CGFloat x = (screenBounds.size.width - w) / 2.0;
    CGFloat y = screenBounds.size.height * yRatio;
    self.cutoutView.frame = CGRectMake(x, y, w, h);
}
